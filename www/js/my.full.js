...
function showDropMenu(objectType, objectID, module, method, extra) {
	...
	if (!li.data('showagain')) {
		...
		$('#dropMenu, #currentItem').click(function (e) {
			e.stopPropagation();
		});
	}
 ...
 */
function autoCheck() {
	$(document).on('click', '.tablesorter tr :checkbox', function () {
		clickInCheckbox = 1;
	});
...
function setModal() {
	...
	function showIframeModal(settings) {
		...
		if (options.waittime > 0) {
			options.waitingFuc = setTimeout(function () {
	...
	$.extend({
		ajustModalPosition: ajustModalPosition, modalTrigger: showIframeModal, colorbox: function (setting) {
			if ((typeof setting == 'object') && setting.iframe) {
				$.modalTrigger({
					type: 'iframe',
					width: setting['width'],
					afterHide: setting['onCleanup'],
					url: setting['href']
				});
			}
		}
			...
			afterHide: function () {
				...
				if (typeof(replaceID) == 'string' && replaceID.length > 0) {
					...
					$('#tmpDiv').load(link + ' ' + idQuery, function () {
						$('#tmpDiv').replaceWith($('#tmpDiv').html());
						setTimeout(function () {
							setModal4List(triggerClass, replaceID, callback, width);
						}, 150);
						...
						$list.find('tbody tr:not(.active-disabled) td').click(function () {
...
function setTableBehavior() {
	$('#wrap .table:not(.table-data, .table-form, .active-disabled)').on('click', 'tbody tr:not(.active-disabled) td', function () {
		$(this).closest('tr').toggleClass('active');
	});
